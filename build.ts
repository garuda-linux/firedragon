...
async function decompressBin() {
  try {
    ...
    if (!(await isExists(binArchive))) {
      console.log(
        chalk.yellow(
          `⚠️  ${binArchive} not found. We will download ${await getBinArchive()} from latest release.`,
        ),
      );
...
async function downloadBinArchive() {
  const releaseDownloadUrl = `https://gitlab.com/garuda-linux/firedragon/firedragon12/-/releases/permalink/latest/downloads/${binArchive}`;
  console.log(chalk.blue(`⬇️ Downloading from release: ${releaseDownloadUrl}`));
  try {
    await $`curl -L --fail --progress-bar -o ${binArchive} ${releaseDownloadUrl}`;
    console.log(chalk.green("✅ Download complete from release!"));
  } catch (error: unknown) {
    console.error(
      chalk.red("❌ Release download failed:"),
      (error as { stderr: string }).stderr,
    );
    throw (error as { stderr: string }).stderr;
  }
  await decompressBin();
}
