<script lang="ts" setup>
    ...
    const hiddenTabIcon = await useBoolPref('userChrome.hidden.tab_icon');
    const hiddenTabIconAlways = await useBoolPref('userChrome.hidden.tab_icon.always');
    ...
    const hiddenSidebarHeader = await useBoolPref('userChrome.hidden.sidebar_header');
    const hiddenSidebarHeaderVerticalTabOnly = await useBoolPref('userChrome.hidden.sidebar_header.vertical_tab_only');
    const hiddenSidebarHeaderType = computed({
        get: () =>
            !hiddenSidebarHeader.value
                ? 'none'
                : hiddenSidebarHeaderVerticalTabOnly.value
                  ? 'verticalTabOnly'
                  : 'default',
        set: (value) => {
    ...
    const hiddenBookmarkbarIcon = await useBoolPref('userChrome.hidden.bookmarkbar_icon');
    const hiddenBookmarkbarLabel = await useBoolPref('userChrome.hidden.bookmarkbar_label');
    ...
    <q-card>
        ...
        <q-list class="q-mb-lg">
            <Item :title="t('pages.design-lepton.hidden.hiddenTabIcon.title')">
                <TypeSelect
                    v-model="hiddenTabIconType"
                    :options="['none', 'default', 'always']"
                    translation-key="pages.design-lepton.hidden.hiddenTabIcon.type"
                />
            </Item>
            <BoolPrefItem
                pref="userChrome.hidden.tab_icon"
                :title="t('pages.design-lepton.hidden.hiddenTabbar.title')"
            />
            ...
            <Item :title="t('pages.design-lepton.hidden.hiddenSidebarHeader.title')">
                <TypeSelect
                    v-model="hiddenSidebarHeaderType"
                    :options="['none', 'default', 'verticalTabOnly']"
                    translation-key="pages.design-lepton.hidden.hiddenSidebarHeader.type"
                />
            </Item>
            <BoolPrefItem
                pref="userChrome.hidden.urlbar_iconbox"
                :title="t('pages.design-lepton.hidden.hiddenUrlbarIconbox.title')"
            />
            <Item :title="t('pages.design-lepton.hidden.hiddenBookmarkbar.title')">
                <TypeSelect
                    v-model="hiddenBookmarkbarType"
                    :options="['none', 'icon', 'label']"
                    translation-key="pages.design-lepton.hidden.hiddenBookmarkbar.type"
                />
            </Item>
            <BoolPrefItem
                pref="userChrome.hidden.disabled_menu"
                :title="t('pages.design-lepton.hidden.hiddenDisabledMenu.title')"
            />
        </q-list>
    </q-card>
</template>
