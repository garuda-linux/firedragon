...
@Component({
    selector: 'app-search',
    imports: [FormsModule, NgOptimizedImage, TranslocoDirective, Button, AutoComplete],
...
export class SearchComponent implements OnInit {
    ...
    async autocomplete($event: AutoCompleteCompleteEvent) {
        const searchEngine = this.searchEngine();
        if (searchEngine) {
            const suggestions = await this.searchService.fetchSuggestions(searchEngine.id, this.searchTerm());
            this.suggestions.set([...suggestions.local, ...suggestions.remote].map((entry) => entry.value));
        } else {
            this.suggestions.set([]);
        }
    }
