...
import { type ElementRef, Injectable, type Renderer2, effect, inject, signal } from '@angular/core';
import { Title } from '@angular/platform-browser';
import { TranslocoService } from '@jsverse/transloco';
import { usePreset } from '@primeng/themes';

import { type AppTheme, backgroundColors, scrollbarColors, themes } from '../app/theme';
import { WallpaperService } from '../app/wallpaper/wallpaper.service';
...
export class ConfigService {
     ...
     * @param write Whether to write the value FireDragon's settings store.
     */
    ...
    async restoreSettings(file: File, renderer: Renderer2, el: ElementRef): Promise<void> {
        ...
        try {
            ...
            Services.prefs.setStringPref(this.pref, JSON.stringify(settings));
        } catch (error) {
    ...
    private async initStore(): Promise<Partial<AppSettings>> {
        ...
        const savedSettings: string | null = Services.prefs.getStringPref(this.pref);
